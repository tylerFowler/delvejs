...
const detectDebuggerExitedError = err => {
  if (err.toString().match(/Process [0-9]+/i).length > 0) {
    let exitStatusMatches = err.toString().match(/(?!status\ )[0-9]+$/);
    let exitStatus = exitStatusMatches.length > 0
      ? exitStatusMatches[0] : 'unkown';

    return Promise.reject(new DelveError.DebuggerExited({exitStatus}));
  }
...
/**
  @name Delve#endSession
  @desc Closes the client connection, optionally detaching from the process
    Note that this will always resolve and will always end the underlying
    socket connection, so it is safe to use this as a fire and forget method
  @param { Boolean } shouldDetach :optional
**/
Delve.prototype.endSession =
function endSession(shouldDetach) {
  let p;
  if (shouldDetach) p = this.detachProcess();
  else p = Promise.resolve(true);

  // ignore whether this fails or not since the server could be closed
  p.catch(() => {}).finally(() => this.rpcClient.end());
};
...
function next() {
  ...
  }).catch(detectDebuggerExitedError);
};
...
function restartProcess() {
  return this._makeCall('Restart').then(() => this.getDebuggerState());
};
...
/**
  @name Delve#setBreakpointCondition
  @desc Sets the condition for the breakpoint w/ the given ID
  @param { Number } id
  @param { String } cond => any boolean Go expression
  @returns Promise<>
**/
Delve.prototype.setBreakpointCondition =
function setBreakpointCondition(id, cond) {
  return this.getBreakpointById(id)
  .then(bp => {
    bp.Cond = cond;
    return this._makeCall('AmendBreakpoint', bp);
  });
};
...
function setSymbol(symbol, val, goroutineId, frameId) {
  let Scope = types.Scope.makeScope(goroutineId, frameId);
  return this._makeCall('SetSymbol', { Scope, 'Symbol': symbol, Value: val });
};
...
function getDebuggerState() {
  ...
  // correct for the 'threads' prop, which comes out with the key 'Threads'
  .then(state => {
